...
interface PhoneProps {
  ...
  triviaView?: 'pre-game-host' | 'pre-game-participant' | 'game' | 'feedback-correct' | 'feedback-wrong' | 'game-settings';
  onTriviaViewChange?: (view: 'pre-game-host' | 'pre-game-participant' | 'game' | 'feedback-correct' | 'feedback-wrong' | 'game-settings') => void;
}
...
export function Phone({ onAppChange, triviaView = 'pre-game-host', onTriviaViewChange }: PhoneProps = {}) {
  ...
  const renderApp = () => {
    switch (currentApp) {
      case 'trivia':
        return <TriviaApp onNavigateToHome={navigateToHome} viewMode={triviaView} onViewModeChange={onTriviaViewChange} />;
      case 'calculator':
