...
/**
 * Trivia Circle Answer Text Display Component
 * 
 * This component attaches to a text gizmo and displays the first answer choice (circle)
 * for trivia questions. It syncs with the TriviaQuestionText component.
 * 
 * Usage:
 * 1. Attach this script to a text gizmo in your world
 * 2. Position it where you want the circle answer to appear
 * 3. The answer will automatically sync with trivia questions
 */
export class TriviaCircleText extends hz.Component<typeof TriviaCircleText> {
  
  static propsDefinition = {
    // Display settings
    fontSize: { type: hz.PropTypes.Number, default: 24 },
    textColor: { type: hz.PropTypes.String, default: "#FFFFFF" },
    backgroundColor: { type: hz.PropTypes.String, default: "#FF4444" }
  };
  ...
  async start() {
    console.log("TriviaCircleText: Starting up...");
    
    // Get reference to the text gizmo this component is attached to
    this.textGizmo = this.entity.as(hz.TextGizmo);
    if (!this.textGizmo) {
      console.error("TriviaCircleText: This component must be attached to a text gizmo!");
      return;
    }
  ...
  private onQuestionStart(eventData: { question: any, questionIndex: number, timeLimit: number }): void {
    ...
    if (question && question.answers && question.answers.length > 0) {
      this.currentAnswer = question.answers[0].text;
      this.isCorrect = question.answers[0].correct;
  ...
  private onQuestionResults(eventData: { question: any, correctAnswerIndex: number, answerCounts: number[], scores: { [key: string]: number } }): void {
    ...
    const answerCount = eventData.answerCounts[0] || 0;
    const isCorrectAnswer = eventData.correctAnswerIndex === 0;
...
hz.Component.register(TriviaCircleText);
